...
local QuestEventHandler = QuestieLoader:ImportModule("QuestieEventHandler")
local QuestieEventHandler = QuestEventHandler
...
local function parseQuestObjective(text)
    local normalized = string.gsub(text, "\239\188\154", ":")
    local objectiveName, numFulfilled, numRequired

    -- Standard: "Name: 1/5"
    objectiveName, numFulfilled, numRequired = string.match(normalized, "(.*):%s*([%d]+)%s*/%s*([%d]+)")
    if objectiveName then return objectiveName, numFulfilled, numRequired end

    -- Ascension Variant 1: "1/5 Name"
    numFulfilled, numRequired, objectiveName = string.match(normalized, "^([%d]+)%s*/%s*([%d]+)%s*(.*)$")
    if objectiveName then return objectiveName, numFulfilled, numRequired end

    -- Ascension Variant 2: "Name 1/5"
    objectiveName, numFulfilled, numRequired = string.match(normalized, "^(.*)%s+([%d]+)%s*/%s*([%d]+)$")
    if objectiveName then return objectiveName, numFulfilled, numRequired end

    -- Ascension Variant 3: "Name (Complete)"
    if string.find(normalized, "%(Complete%)") then
        ...
        if questLogIndex then
            ...
            for i = 1, numObjectives do
                ...
                if objectiveType ~= "log" then
                    ...
                    local finished = isCompleted
                    if (not finished) and numFulfilled and numRequired and (tonumber(numFulfilled) >= tonumber(numRequired)) then
                        finished = true
                    end
...
function QuestieCompat:ADDON_LOADED(event, addon)
    ...
    -- Project Ebonhold Corrections
    -- Ebonhold is a roguelite server where quests reset on death, but are marked as repeatable in the DB
    -- We need to clear the repeatable flag so completed quests are tracked properly
    local realmName = GetRealmName()
    if realmName == "Ebonhold" then
        QuestieCompat.RegisterCorrection("questData", function()
            return {
                -- Quest 208: Big Game Hunter - incorrectly flagged as repeatable
                [208] = {
                    [QuestieDB.questKeys.specialFlags] = 0, -- Clear repeatable flag
                },
                -- Add more quest corrections here as needed
            }
        end)
    end
end
