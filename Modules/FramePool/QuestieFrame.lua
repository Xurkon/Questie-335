...
function QuestieFramePool.Qframe:New(frameId, OnEnter)
    ...
    -- Glow is a Texture (not a Frame) so it can stack correctly inside ScrollFrames


    local glow = newFrame:CreateTexture(nil, "BACKGROUND")


    glow:SetTexture(Questie.icons["glow"])


    glow:SetBlendMode("BLEND")


    glow:SetDrawLayer("BACKGROUND", 0)
    ...
    glow:SetPoint("CENTER", newFrame, "CENTER", 0, 0)
...
newFrame:SetFrameStrata("FULLSCREEN");
    ...
    -- Make sure the icon itself always draws above the glow
    newTexture:SetDrawLayer("OVERLAY", 1)
if not QuestieCompat.Is335 then
        newTexture:SetTexelSnappingBias(0)
        newTexture:SetSnapToPixelGrid(false)
    end

    local glowt = newFrame.glow
